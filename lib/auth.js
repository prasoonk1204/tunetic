    ...
    callbacks: {
        async signIn({ user, account, profile }) {
            ...
            if (existingUser) {
                ...
                const providerName =
                    user.name || profile?.name || profile?.data?.name;
                const providerImage =
                    user.image ||
                    profile?.picture ||
                ...
                if (
                    providerUsername &&
                    existingUser.username !== providerUsername
                ) {
        ...
        async session({ session, token }) {
            ...
            const user = await User.findOne({
                ...
                "_id name email image username provider createdAt updatedAt",
            );
